...
class Vitally_Responsible {
    ...
    public static function vital_the_content( $more_link_text = null, $strip_teaser = false) {
	    ...
	    echo $content;
	    if($cached)
	    	echo '<!-- content retrieved from cached post meta by Vitally Responsible -->';
	}
    		...
    		foreach ($reg_images[0] as $key => $value) {
		        ...
		        foreach ($images as $img) {
	                ...
	                if ( ! preg_match( $ignorelist, $classes ) ) {
	                    ...
	                    $picturefill = 	'<span data-picture data-alt="'. $alt .'" class="picturefill-wrap '. $classes .'" title="' . $title . '">';
